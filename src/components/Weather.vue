<template>
  <div class="container max-w-screen-xl p-4 mx-auto">
    <h1 class="mb-6 text-3xl text-center text-white">
      Weather App
    </h1>
    ...
    <div
      ...
      class="w-full p-4 mx-auto mb-12 bg-white border-2 rounded-lg md:w-3/5 border-cyan-800"
    >
      <p class="mb-3 font-sans text-base text-center capitalize text-cyan-700">
      ...
      <div
        v-if="weather"
        class="flex gap-4 p-4 mb-3 border-2 border-cyan-800"
      >
        <WeatherCurrentCard
          ...
          :name-icon="weather.weather[0].icon"
          :description-icon="weather.weather[0].description"
        />
      </div>
      <p class="mb-3 text-lg font-medium text-cyan-800">
        Forecast
      </p>
      <div
        v-for="(datas, index) in weather_forecast"
        :key="index"
        class="mb-8"
      >
        <button
          ...
          @click="datas.isHidden = !datas.isHidden"
        >
          {{
        ...
        <div class="flex flex-wrap justify-center gap-4">
          ...
          >
            <div
              ...
              :key="index"
              class="p-4 mb-6 border-2 border-cyan-800"
            >
              <WeatherCard
                :hour="item.dt"
                ...
                :name-icon="item.weather[0].icon"
                :description-icon="item.weather[0].description"
              />
            </div>
          </div>
        </div>
    ...
    <div
      v-if="weather_position_forecast"
      v-show="!weather"
      class="w-full p-4 mx-auto mb-12 bg-white border-2 rounded-lg md:w-3/5 border-cyan-800"
    >
      <p class="mb-3 font-sans text-base text-center text-cyan-700">
        Current weather in your location : <span
          class="text-xl"
        >{{ city }}</span>
      </p>
      <div
        v-if="weather_position"
        class="flex gap-4 p-4 mb-3 border-2 border-cyan-800"
      >
        <WeatherCurrentCard
          :temp="weather_position.main.temp.toFixed()"
          :feels-like="weather_position.main.feels_like.toFixed()"
          :humidity="weather_position.main.humidity"
          :wind="weather_position.wind.speed"
          :name-icon="weather_position.weather[0].icon"
          :description-icon="weather_position.weather[0].description"
        />
      </div>
      <p class="mb-3 text-lg font-medium text-cyan-800">
        Forecast
      ...
      <div
        v-for="(datas, index) in weather_position_forecast"
        :key="index"
        class="mb-8"
      >
        <button
        ...
        <div class="flex flex-wrap justify-center gap-4">
          ...
          >
            <div
              v-for="(item, index) in items"
              :key="index"
              class="p-4 border-2 border-cyan-800"
            >
              <WeatherCard
                :hour="item.dt"
                :temp="item.main.temp.toFixed()"
